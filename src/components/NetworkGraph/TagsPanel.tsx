...
import { CheckCircledIcon, CircleIcon, HamburgerMenuIcon } from "@radix-ui/react-icons";
...
const TagsPanel: FC<{
  ...
  return (
    <Panel
      title={
        <>
          <HamburgerMenuIcon className="text-muted" /> Categories
    ...
    >
      ...
      <p className="buttons">
        <button className="btn" onClick={() => setTags(mapValues(keyBy(tags, "key"), () => true))}>
          <CheckCircledIcon /> Check all
        </button>{" "}
        <button className="btn" onClick={() => setTags({})}>
          <CircleIcon /> Uncheck all
        </button>
      </p>
      <ul>
        {sortedTags.map((tag) => {
